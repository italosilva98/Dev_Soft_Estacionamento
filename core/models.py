...
class Veiculo(models.Model):
    marca = models.ForeignKey(Marca, on_delete=models.CASCADE) #chave estrangeira
...
class MovRotativo(models.Model):
    ...
    def horas_total(self):
        if self.checkout != None:
            total_hora = math.ceil((self.checkout - self.checkin).total_seconds() / 3600)
        else:
            total_hora = math.ceil((self.hora - self.checkin).total_seconds() / 3600)
        return total_hora
